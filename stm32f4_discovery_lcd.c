...
void LCD_DisplayCharTransparency(uint16_t Line, uint16_t Column, uint8_t Ascii)
{
  Ascii -= 32;
  LCD_DrawCharTransparency(Line, Column, &LCD_Currentfonts->table[Ascii * LCD_Currentfonts->Height]);
}

void LCD_DrawCharTransparency(uint16_t Xpos, uint16_t Ypos, const uint16_t *c)
{
...
/**
  ...
  * @retval None
  */
void LCD_DisplayStringXY(uint16_t x,uint16_t y, uint8_t *ptr,uint8_t transparency)
{
  /* Send the string character by character on lCD */
  while (*ptr != 0 && x < LCD_PIXEL_WIDTH)
  {
    /* Display one character on LCD */
	  if(transparency == 1)
		  LCD_DisplayCharTransparency(y, x, *ptr);
	  else
		  LCD_DisplayChar(y, x, *ptr);
    /* Decrement the column position by 16 */
